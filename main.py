...
"""
GBase v0.7.0/main.py

GBase 核心启动入口。

Identity: 通过 IDENTITY 环境变量或 --identity 参数设置。
Platform: 通过 --platform 或子命令选择 (feishu / cli / web)。
...
from lib.compat import get_port_owner, kill_process, IS_WINDOWS
...
def _auto_route_model(user_message: str, default_model: str) -> str:
    ...
    logger.info("自动路由: 复杂度=%s → 模型=%s (原默认=%s)", complexity, routed, default_model)
    return routed
...
def _check_lifeline():
    ...
    if snaps:
        logger.info("LIFELINE: HEAD=%s, 最近 5 个快照:", commit)
        for s in snaps[:5]:
            logger.info("  - %s | %s | %s", s["tag"], s["reason"][:30], s["timestamp"][:19])
    else:
        logger.info("LIFELINE: HEAD=%s, 暂无快照", commit)
...
def _init_sandbox():
    ...
    for rel in checks:
        fp = base / rel
        if not fp.exists():
            logger.warning("🚫 沙箱推演: 关键文件缺失 %s", fp)
        else:
            issues = lint(str(fp))
            if issues:
                for issue in issues:
                    logger.warning("  ⚠ 版本兼容: %s", issue)
    logger.info("沙箱推演: 关键文件完整性检查通过")


def _setup():
    """启动时的初始化（Tools 扫描 + 注册 + 安全推演检查 + 模块初始化）。"""
    auto_scan("tools")
    register_default()
    _check_lifeline()
    _init_sandbox()
    _setup_log_rotation()
    _init_restored_modules()


def _init_restored_modules():
    ...
    try:
        ...
        logger.warning("Skill Router init failed: %s", e)
    ...
    try:
        ...
        logger.info("DAG Orchestrator: initialized with %d pilot workflows", len(dag_orch._task_keywords))
    except Exception as e:
        logger.warning("DAG Orchestrator init failed: %s", e)
    ...
    try:
        ...
        logger.warning("Evolution Engine init failed: %s", e)


async def _startup_guard(storage, mirror, channel, port):
    """启动 5 秒后执行自检：记忆健康 + 端口可达。"""
    await asyncio.sleep(5)
    errors = []
    try:
        if storage and storage._conn:
            k = storage._conn.execute("SELECT COUNT(*) FROM entries WHERE type='knowledge'").fetchone()[0]
            e = storage._conn.execute("SELECT COUNT(*) FROM entries WHERE type='experience'").fetchone()[0]
            logger.info("🧠 记忆健康: knowledge=%d, experience=%d", k, e)
        ...
        for retry in range(3):
            try:
                r = _httpx.get(f"http://127.0.0.1:{port}/health", timeout=3)
                if r.status_code == 200:
                    logger.info("✅ 端口自检: 127.0.0.1:%d/health → %d", port, r.status_code)
                    port_ok = True
                    break
            except Exception:
                if retry < 2:
...
async def _periodic_wal_checkpoint(storage):
    """周期性 WAL checkpoint，防止 WAL 文件无限增长。"""
    while True:
...
async def _run_web(port: int = 8775):
    ...
    try:
        ...
        logger.error("WebChat 频道未找到，请确认 lib/channels/webchat.py 存在")
        print("❌ Error: WebChat 频道模块缺失")
        return

    logger.info("━━━━━━━━━━━━━━━━━━━")
    logger.info("GBase v0.7.0 Web Chat 已启动")
    logger.info("端口: %d, 模型: %s", port, model)
...
async def cli_mode(identity_name: str = "default"):
    ...
    # ── 初始化鉴面引擎（_data_dir 有值则使用独立的 Armor mirror.db） ──
    ...
    logger.info("Skill 加载: %s 个 (%s)", len(skill_names), ", ".join(skill_names) if skill_names else "无")
    ...
    # ── 创建 LLM 内核（优先级：MiniMax > 阿里云百炼 > DeepSeek） ──
    model = _resolve_model()
    ...
    print(f"\n🤖 GBase v0.7.0 — Identity: {identity_name} ({routed_model})")
    print("输入 /quit 退出\n")
...
def _ensure_port_free(port: int):
    ...
    使用 lib.compat 跨平台兼容层（Windows/macOS/Linux）。
    """
    import socket
    ...
    try:
        ...
        pids = [pid] if pid and pid != my_pid else []
        ...
        for pid in pids:
            ...
            kill_process(pid, force=False)  # SIGTERM 等效
        ...
        for pid in pids:
            kill_process(pid, force=True)  # SIGKILL 等效
        _time.sleep(1)
    except Exception as e:
...
async def feishu_mode(identity_name: str = "default", port: int = 8420, data_dir: str = None):
    ...
    # ── 初始化鉴面引擎（data_dir 有值则使用独立的 Armor mirror.db） ──
    ...
    if MOD_RSI in _edition.modules and scheduler and auto_learner:
        try:
            ...
            if not has_learn:
                ...
                logger.info("已注册自主学习定时任务 (启动后60秒首次, 极客版)")
            else:
                logger.info("自主学习定时任务已存在，跳过")
        except Exception as e:
            logger.error("注册自主学习定时任务失败: %s", e)
    else:
        logger.info(
            "跳过自主学习定时任务 (版本=%s, scheduler=%s, learner=%s)",
    ...
    if MOD_PROJECT_MEMORY in _edition.modules and scheduler:
        try:
            ...
            if not has_daily:
                first_utc = time.time() + 300  # 5分钟后第一次触发，之后每24小时
                if data_dir:
                    ...
                    logger.info("已注册 Armor 每日记忆提取任务 (%s, 启动后5分钟首次)", arm_name)
                else:
                    scheduler.add_job(
                        {"type": "every", "interval": 86400, "first_run": first_utc},
                        "【每日记忆提取】",
                    )
                    logger.info("已注册每日记忆提取任务 (启动后5分钟首次)")
            else:
                logger.info("每日记忆提取任务已存在，跳过")
        except Exception as e:
            logger.error("注册每日记忆提取任务失败: %s", e)
    else:
        logger.info("跳过每日记忆提取任务 (版本=%s, scheduler=%s)", _edition.name, bool(scheduler))

    # ── 注册离线巩固周期（睡眠模块） ──
    if scheduler and hasattr(scheduler, "register_callback"):
        try:
            ...
            logger.info("sleep_cycle 模块未找到，跳过离线巩固周期")
        except Exception as e:
            logger.error("注册 sleep 周期失败: %s", e)

    # FastAPI 应用
    app = FastAPI(title="GBase v0.7.0")
    ...
    async def fission_vote(candidate: str = "", term: int = 0):
        """联邦选举投票端点。其他节点在无法连接时，询问是否同意该节点当选 leader。"""
    ...
    async def lifeline_snapshot_before_edit(request: Request):
        ...
        # 同时记录到日志
        logger.info(
            "LIFELINE: 快照完成 — tag=%s, commit=%s, git_ok=%s, backup_ok=%s",
            result.get("tag", ""),
            result.get("commit", ""),
    ...
    async def audit_handler(request: Request):
        ...
        try:
            ...
            elapsed = round(time.time() - start, 1)
            err_msg = f"任务超时（{max_sec or '不限'}秒限制，实际{elapsed}秒）"
            ...
            logger.error("[协议] %s 任务执行失败: %s", identity_name, e)
            cb_payload = make_callback_payload(
                task_id=body.get("task_id", ""),
                task_type=task_type,
                status="failed",
    ...
    async def hammer_audit(request: Request):
        """Hammer 路由。仅 hammer identity 可访问。"""
    ...
    async def ink_evaluate(request: Request):
        """Ink 路由。仅 ink identity 可访问。"""
        if "ink" not in str(identity_name).lower():
            return {"error": f"This route is for ink identity only. Current: {identity_name}"}
        return await audit_handler(request)

    logger.info("飞书模式启动在 0.0.0.0:%d", port)

    # ── 版本启动横幅 ──
    logger.info("=" * 50)
    logger.info(
        "  GBase v0.7.0 %s (%s) 已启动", _edition.label if _edition else "hacker", _edition.name if _edition else "hacker"
    )
    ...
    # ── 启动自检守护 ──
    asyncio.create_task(_startup_guard(storage, mirror, channel, port))

    # ── 周期性 WAL checkpoint ──
    asyncio.create_task(_periodic_wal_checkpoint(storage))
...
def main():
    ...
    if "--mode" in sys.argv:
        ...
        if len(sys.argv) > idx + 1 and sys.argv[idx + 1] == "web":
            web_port = int(os.environ.get("GBASE_WEB_PORT", "8775"))
            if "--port" in sys.argv:
                pidx = sys.argv.index("--port")
    ...
    if "--arm" in sys.argv:
        ...
        if not cfg:
            print(f"未知 Armor: {arm_name}，可选: hammer, ink, bumblebee, laser, forge")
            return
        ...
        # ── 按身份分配模型 ──
        # ink 用 MiniMax M3（擅长编程/图形），其他用 qwen3.7-plus
        if arm_name == "ink":
            os.environ["OPPRIME_MODEL"] = "minimax"
            logger.info("Armor %s → 模型: MiniMax-M3 (ink 专用)", arm_name)
        else:
            # 移除可能存在的 MiniMax 覆盖，走阿里云百炼默认
            if os.environ.get("OPPRIME_MODEL", "") == "minimax":
                del os.environ["OPPRIME_MODEL"]
            logger.info("Armor %s → 模型: qwen3.7-plus (默认)", arm_name)

        # ── 注入 Armor 角色守卫 ──
        set_global("arm_role", arm_name)
        # Armor 也走沙箱（共用底座 lib/sandbox_safety.py）
    ...
    if platform == "cli":
        ...
        print("       python3 main.py --web [--port 8765]    # Web 聊天模式")
        print(f"版本: {list_editions()}")


# ── 默认 Armor system prompts ──
...
- 复杂逻辑加注释
- 不输出未验证的方案
- 完成的代码**再读一遍、再改一遍**：变量名准确吗？函数职责单一吗？有重复可以消除吗？

### 🎯 极致追求 — 交付前的五问自审
每次交付代码前，停一下，五问自己：
1. 每一行都有存在的理由吗？（有没有死代码、临时调试代码、debug 遗留？）
2. 读起来自然吗？（不用跳着读就能理解逻辑流？）
3. 变量/函数名看一眼就知道做什么吗？（需要读注释才懂的逻辑，是命名不够好）
4. 异常的边界考虑了吗？（空值、超时、并发、非法输入？）
5. 你自己满意吗？（不满意就再改一版，不要交货给你自己都不喜欢的代码）

### 🛡️ 强制验证闭环（必须执行，不可跳过）
每写/改完一个文件后，必须做以下三步：
1. **语法检查**：`python3 -m py_compile <file>` 或直接确保语法正确
2. **验证工具**：调用 `forge_verify(file_path)` 做六项全面检查
...
**核心原则：先探索，后评估。exec 就是你通向宿主机信息的眼睛。**

## 🛡️ 上下文稳压协议（强制执行）

你的思考会随工具调用链拉长而漂移。执行任何任务前，先读 `/tmp/hammer_step1.json` 或 `/tmp/ink_step1.json` 确认当前状态，
然后按以下三步走: 读项目(JSON中间态)→跑测试(JSON中间态)→写报告(基于JSON数据)，每步超3次工具调用无结果就截断并记录。
""",
    "ink": """# Identity
...
**核心原则：先验证，后评估。exec 就是你的浏览器和测试工具。**

## 🛡️ 上下文稳压协议（强制执行）

你的思考会随工具调用链拉长而漂移。执行任何任务前，先读 `/tmp/ink_step1.json` 或 `/tmp/hammer_step1.json` 确认当前状态，
然后按以下三步走: 读项目(JSON中间态)→做测试(JSON中间态)→写评估(基于JSON数据)，每步超3次工具调用无结果就截断并记录。
""",
}
