...
async def stop_mock_server(port: int = 4000) -> dict:
    ...
    if server:
        try:
            from lib.compat import kill_process
            kill_process(server["pid"], force=False)
        ...
        return {
            "success": False,
            "message": f"No active mock server on port {port}. Use lib.compat.kill_process() or taskkill/netstat to force stop.",
        }
