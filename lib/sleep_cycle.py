...
# ── 默认配置 ──
# Enterprise mode (default): preserve all data
# Lite mode: aggressive pruning (v0.5.1 style)
import os as _os
_MODE = _os.environ.get("GBASE_MODE", "enterprise")

DEFAULT_WINDOW_HOURS = 168  # 回顾最近 7 天的 session
COMPRESSION_TTL = 1209600  # 14 天无更新的 session 不处理（秒）

if _MODE == "lite":
    IMPORTANCE_FLOOR = 0.15    # 激进剪枝：低于此值的 mirror 条目标记为噪声
    MAX_SESSION_ROWS = 15       # 激进压缩：每 session 压缩时保留的条目数
else:
    IMPORTANCE_FLOOR = 0.0     # 企业模式：不修剪 mirror
    MAX_SESSION_ROWS = 100000   # 企业模式：不截断 session
