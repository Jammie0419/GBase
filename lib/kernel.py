...
"""
GBase kernel loop module / GBase 内核循环模块

三层架构的第二层：
- 单一职责：LLM 调用 + tool_call 执行循环
- 不负责：记忆注入、经验存储、scout、认知检测
- 最多 5 层工具调用深度
"""
...
# ── GMem 集成钩子 ──
# GMem 是 GBase 的原生记忆系统，通过升级 mirror/toolkit/experience 三个子模块实现
# 不依赖外部服务，不引入新依赖
# P0: KV Cache 准备 → hot_pattern_observe() 追踪高频模式
# P1: 异步记忆调度 → create_task 非阻塞经验提取 + async_record
# P2: 经验标准化 → export/import 版本验证 + 过滤
...
async def _async_mirror_record(mirror_engine, user_message: str, reply: str, completed_ok: bool = True):
    """后台记录 mirror 记忆。空实现 - 记录决策完全委托给 agent。"""
...
async def _auto_note_if_deep_work(tool_count: int, reply: str, user_message: str):
    ...
    这样做的理由：
    - 任务不会在回合结束时自动触发 note_write
    - 热记忆（mirror）会衰减，深度研究/设计内容重启后只会是片段
    - L4 笔记不衰减，是唯一可靠的持久层
    - 与其依赖 LLM 主动记得调用 note_write，不如让系统自动兜底
    - 但 LLM 主动写的（有判断）远好于自动的，所以自动只作为兜底，不替代手写
    """
    # 条件 1：工具调用次数足够
    if tool_count < 5:
        return
...
async def _auto_persist_experience(mirror_engine, user_message: str, reply: str, completed_ok: bool = True):
    """每轮对话后自动沉淀经验到 mirror.db。

    通过 mirror.record() 写入 type='experience' 条目，让 agent 在未来的 session 中
    记住它构建过什么，即使重启后也不会丢失。无阈值过滤；
    置信度过滤由 mirror 自身的评分和衰减机制处理。
    """
...
# ── RSI 双旋钮：任务意图分类 ──
# 这是在特定 GBase 实例上的受控实验。
# 此处的更改会影响 gbase/ 中所有共享此配置的 GBase 实例。
# TODO: 实验验证后发布到 gbase-release。
_TASK_TYPES = {
...
class Kernel:
    ...
    def __init__(
        ...
        # 🧪 实验 #1 — OOD 温度匹配
        self._ood_similarity_threshold = 0.3  # 低于此值 → 未知领域
        self._ood_exploit_threshold = 0.6  # 高于此值 → 已知模式

        # 🧪 实验 #2 — 持续梯度累积
        self._gradient_log: list[dict] = []
        self._gradient_trigger_count = 5  # 达到此数量后触发 RSI
        ...
        # ── ArchiveStore 初始化（无 session 依赖，全局写入 + 全局搜索） ──
    ...
    def _build_dynamic_system_prompt(self) -> str:
        """动态构建 system prompt：基础身份 + workspace 文件注入 + skill 索引。
        ...
        # ── GMem Phase A2: Archive Store 上下文检索（替代压缩：全量存档，按需搜索） ──
        if self._archive_store and self._current_user_message:
            try:
                ...
                logger.exception("Archive 搜索失败（不阻塞）")
    ...
    async def run(
            ...
            try:
                ...
                if _keywords:
                    ...
                    if archive_lines:
                        ...
                        logger.info("ArchiveStore 全局搜索到 %d 条相关记忆", len(archive_lines))
            except Exception as e:
                logger.warning("ArchiveStore 搜索失败（不影响主流程）: %s", e)
        ...
        # ── 2. 构建 messages（含 ArchiveStore 搜索结果注入） ──
        messages: list[dict] = []
        if session:
        ...
        # 最终 user message（已含 archive 搜索结果）
        final_user = (
        ...
        if reply and len(reply) > 10:
            ...
            # 🧪 实验 #2 — 记录本轮梯度
            self._record_gradient(user_message, reply, _this_turn_tools)
            # 📊 RSI: 实时工具调用成功率追踪（计数器）
            self._tool_call_count = getattr(self, "_tool_call_count", 0) + _this_turn_tools
    ...
    def _estimate_ood(self, message: str) -> float | None:
        """评估此查询的已知程度（与 mirror 记忆对比）。

        返回相似度 (0-1, 1=与过去非常相似)，如果 mirror 不可用则返回 None。
        """
    ...
    def _record_gradient(self, message: str, reply: str, tool_count: int):
        ...
        累积直到触发数量，然后记录日志。不触发真实 RSI
        （那仍由 evolution_engine 在文件变更时完成）。

        阶段 2: 同时写入 RSI 过程指标到 data/metrics/rsi_quality.jsonl
        """
        ...
        if len(self._gradient_log) >= self._gradient_trigger_count:
            ...
            # 在 checkpoint 中包含用户姿态
            entry["stance"] = self._user_stance
            # 清除已记录的条目（保留最近 1 条以避免边界情况）
    ...
    async def _loop(
        ...
        if depth >= MAX_TOOL_DEPTH:
            ...
            if finish_reason == "length":
                logger.warning(
                    "⚠️ LLM 输出被截断 (finish_reason=length)！max_tokens=%s 可能不够", self.max_tokens
                )
                reply += "\n\n[⚠️ 输出被截断，结果可能不完整]"
            if session:
