...
v2.3 — 合并反脆弱元认知升级 + 去重逻辑：
  - _ANTI_FRAGILE_RULES 动态规则集（失败模式 → 改进策略映射）
  - _META_REFLECTION_PROMPT 元认知反思框架（Situation→Action→Outcome→Lesson）
  - 去重逻辑：同一规则在 DEDUP_WINDOW 内不重复记录
  - 队列式批量处理：extract() 入队 + flush() 批量提取
"""
...
class ExperienceEngine:
    ...
    async def flush(self, llm_client=None):
        ...
        for ctx in contexts:
            # 确保 task_theme 存在（队列中的上下文可能缺少此字段）
            if "task_theme" not in ctx:
                import re as _re

                ctx["task_theme"] = _re.sub(r"[^一-鿿\w\s]", "", ctx.get("user_message", "")[:60]).strip() or "未知任务"
    ...
    async def _llm_extract(self, context: dict, client):
        ...
        """
        # 确保 task_theme 存在
        if "task_theme" not in context:
            import re as _re

            context["task_theme"] = _re.sub(r"[^一-鿿\w\s]", "", context.get("user_message", "")[:60]).strip() or "未知任务"
