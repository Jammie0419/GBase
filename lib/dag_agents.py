...
from lib.compat import get_available_memory_kb, get_disk_usage_percent, IS_WINDOWS

_ENV_ROOT = os.getenv("GBASE_ROOT_DIR", ".")
# Security: only allow explicit relative or absolute paths — no empty/unsafe values
from lib.compat import is_absolute_path
if not _ENV_ROOT.strip() or _ENV_ROOT.strip() in ("..", ".") or (not is_absolute_path(_ENV_ROOT) and _ENV_ROOT.strip() not in (".", )):
...
def disk_safety_hook(step: dict, context: dict) -> tuple:
    ...
    try:
        ...
        if pct > 90 and step.get("agent_type") in write_agents:
...
def mem_safety_hook(step: dict, context: dict) -> tuple:
    ...
    try:
        avail_kb = get_available_memory_kb()
        if avail_kb is not None and avail_kb < 200 * 1024:
...
def agent_health_check(inputs: dict, context: dict) -> dict:
    ...
    try:
        if IS_WINDOWS:
            return {"status": "skip", "error": "Shell scripts not supported on Windows"}
