...
核心逻辑：
  ...
  - write_file / exec_command：领地检查 + prompt for admin authorization
  - read_file：只读不写，不阻塞但记录警告

设计原则：
  1. 领地规则在静态注册表中定义，不依赖运行时发现
  2. explicitly instructed by admin（通过对话上下文）的操作不拦截
  3. 代码层只做检测，授权判断交给对话层
"""
...
def build_territory_error(violation: str, abs_path: str, caller_label: str = "操作") -> str:
    ...
    return (
        ...
        f"这是其他 Agent 的领地，without admin authorization不能自行修改。\n"
        f"如需授权，请先获得管理员许可。"
    )
